...
!function(d) {
	var h = function(a) {
		this.messages = {
			defaultMessage: "您输入的值不合法！",
			type: {
				email: "请输入合法的邮箱地址！",
				url: "您输入的URL不合法！",
				urlstrict: "请输入合法的URL地址！",
				number: "输入的值只能是的数字！",
				digits: "输入的值只能是数字！",
				dateIso: "输入的日期格式不合法，正确格式是：(YYYY-MM-DD)！",
				alphanum: "输入的值应该由字母组成！",
				phone: "请输入合法的电话号码！"
			},
			notnull: "您不可以输入空值！",
			notblank: "您不可以输入空白！",
			required: "该项为必填项！",
			regexp: "您输入的内容不符合格式规定！",
			min: "输入的值不得小于 %s！",
			max: "输入的值不得超过 %s！",
			range: "输入的值必须介于 %s和%s之间！",
			minlength: "输入内容过少，至少要包含%s个字符",
			maxlength: "输入内容过多，至多可以输入%s个字符",
			rangelength: "输入的内容长度不合法，输入字符数应该介于 %s~%s之间。",
			mincheck: "您至少要选择 %s个选项",
			maxcheck: "您最多可以选择%s个选项，请去掉多余的选项！",
			rangecheck: "您只能选择%s~%s个选项！",
			equalto: "请保持前后输入的值一致！"
		};
		this.init(a)
	};
